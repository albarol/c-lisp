...
#define clisp_chunk_assert(t, cond, fmt, ...) \
      if (!(cond)) { clisp_chunk_t* error = clisp_chunk_error(fmt, ##__VA_ARGS__); clisp_chunk_free(t); return error; }
...
#define clisp_expr_assert(t, cond, fmt, ...) \
    if (!(cond)) { clisp_chunk_t* error = clisp_chunk_error(fmt, ##__VA_ARGS__); clisp_expr_free(t); return error; }
